 ...
 * void listCar | apresenta uma lista das placas do carros na garagem
 * void removeCar | fluxo de saida de carro da garagem
 * void dateFileName | cria o arquivo com o nome correspondendo aa data (formato
                       YYYY-MM-DD.txt e inclui cabecalho inicial
*/
...
void main()
{
  ...
  dateFileName();                        /** cria e abre arquivo e saída e armazena o nome do arquivo em reportFileName[15] */
...
void addCar() {
  ...
  if (confirm != '1') {
      ...
      if (searchCar(carPlate)!= -1)
          {
          printf ("\n\tPLACA INVALIDA - CARRO JÁ INSERIDO. (Tecle: 1 para corrigir / 2 para voltar): ");
          scanf ("%s", &confirm);
          if (confirm != '1') {
              return;
              }
...
void removeCar() {
  ...
  if(confirm == 1) {
    ...
    if(carFoundIndex == -1) {
      ...
      fprintf (fileP,"%s\t%s\t%dh %dmin\tR$ %5.2f\tR$ %5.2f\n",carPlate,buff,minutes/60, minutes%60,hoursValue,totalValue);
...
strftime(buff, 20, "%Y-%m-%d %H:%M:%S", localtime(&now));
  ...
  fprintf (fileP, "Sistema aberto em (data e hora): %s\n\n", buff);
  fprintf (fileP, "Placa\tData e Hora de Saida\tEstadia\tValor Pago\tSubtotal\n", buff);
  return;
  }
